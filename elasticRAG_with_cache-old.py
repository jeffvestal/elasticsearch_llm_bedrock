...
@st.cache_resource
def initElastic():
    # TODO SET ENV VARS FOR ELASTIC APM
    os.environ['ELASTIC_APM_SERVICE_NAME'] = "elasticsearch_cache_with_bedrock"
    apmclient = elasticapm.Client()
    elasticapm.instrument()

    es = Elasticsearch(
        cloud_id=os.environ['ELASTIC_CLOUD_ID'],
        #cloud_id="Wonder_Wharf:dXMtY2VudHJhbDEuZ2NwLmNsb3VkLmVzLmlvOjQ0MyRhN2JmNDhiZjQyYWQ0MDNhYjQ1ZGQ2YjkwYjg2MGY4NSRhZTQ0YWM5ZjBlODI0ZmYxOTAyY2IxNjdkMGYyOGYwMA==",
        basic_auth=(os.environ['ELASTIC_USER'], os.environ['ELASTIC_PASSWORD']),
        #basic_auth=("bobbelcher, "qGA8ekXs.D_Pq2L_c3WJ"),
        request_timeout=30
    )
...
def genAI(prompt,
    ...
    body = json.dumps(
        {
         'prompt': f"Human: {truncated_prompt}\n\nAssistant:",
         'max_tokens_to_sample': max_context_tokens,
         'temperature': 0.5,
         'top_p': 1,
         'top_k': 250
         }
...
def toLLM(resp, url, usr_prompt, sys_prompt, neg_resp, show_prompt, model):
    ...
    answer = genAI(prompt_formatted, model, sys_content=sys_prompt)
...
cache = ElasticsearchLLMCache(es_client=es,
                              index_name="llm_cache_bedrock",
                              create_index=True  # setting only because of Streamlit behavior
                              )
...
with st.form("chat_form"):
    ...
    with st.expander("Show Prompt Override Inputs"):
        ...
        sys_prompt = st.text_area("create an alernative system prompt", placeholder=system_default,
